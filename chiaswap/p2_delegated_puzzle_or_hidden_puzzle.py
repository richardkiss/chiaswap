...
def calculate_synthetic_offset(public_key: G1Element, hidden_puzzle_hash: bytes) -> int:
    blob = hashlib.sha256(bytes(public_key) + hidden_puzzle_hash).digest()
    offset = int(Program.to(blob))
    offset %= GROUP_ORDER
    return offset


def calculate_synthetic_public_key(public_key: G1Element, hidden_puzzle_hash: bytes) -> G1Element:
...
def calculate_synthetic_secret_key(secret_key: PrivateKey, hidden_puzzle_hash: bytes) -> PrivateKey:
...
def puzzle_hash_for_synthetic_public_key(synthetic_public_key: G1Element) -> bytes:
    public_key_hash = Program.to(bytes(synthetic_public_key)).tree_hash()
    return MOD.curry_hash(public_key_hash)


def puzzle_for_public_key_and_hidden_puzzle_hash(public_key: G1Element, hidden_puzzle_hash: bytes) -> Program:
...
def puzzle_hash_for_public_key_and_hidden_puzzle_hash(public_key: G1Element, hidden_puzzle_hash: bytes) -> bytes:
    synthetic_public_key = calculate_synthetic_public_key(public_key, hidden_puzzle_hash)
...
def puzzle_for_public_key_and_hidden_puzzle(public_key: G1Element, hidden_puzzle: Program) -> Program:
    return puzzle_for_public_key_and_hidden_puzzle_hash(public_key, hidden_puzzle.tree_hash())
...
def puzzle_hash_for_pk(public_key: G1Element) -> bytes:
